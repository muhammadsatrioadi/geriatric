...
@section('admin_layout')
    <div class="col-md-12">
        <form id="edit-form" action="{{ route('admin.pasiens.update', $pasien->id) }}" method="POST"
                        ...
                        <div class="col-md-6 col-lg-4">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-select form-control @error('riwayat_jatuh') is-invalid @enderror"
                                    name="riwayat_jatuh" @if (!$errors->any()) disabled @endif>
                                    <option value="Pernah"
                                        {{ old('riwayat_jatuh', $pasien->riwayat_jatuh) == 'Pernah' ? 'selected' : '' }}>
                                        Pernah
                                    </option>
                                    <option value="Tidak"
                                        {{ old('riwayat_jatuh', $pasien->riwayat_jatuh) == 'Tidak' ? 'selected' : '' }}>
                                        Tidak
                                    </option>
                                </select>
                                @error('riwayat_jatuh')
            ...
            <div class="card">
                ...
                <div class="card-action d-flex gap-2">
                    <div>
                        <button type="button" id="edit-btn" class="btn btn-warning" title="Edit Data Pasien"><i class="fa fa-edit"></i> Edit</button>
                        <button type="submit" id="save-btn" class="btn btn-success"
                            style="display:none"><i class="fa fa-save"></i> Simpan</button>
                        <a href="{{ route('admin.pasiens') }}" class="btn btn-danger"><i class="fa fa-times"></i> Batal</a>
                    </div>
                </div>
            </div>
        </form>

        <!-- Hasil Klasifikasi dan Video -->
        <div class="card">
            <div class="card-header">
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <table class="table table-sm">
                            <tr>
                                ...
                                <td>
                                    @if ($pasien->barthel_index !== null)
                                        @php
                                            $barthelCat = \App\Helpers\PemeriksaanHelper::getBarthelCategory(
                                                $pasien->barthel_index,
                                            );
                                            // Determine badge by category prefix
                                            $badgeClass = str_contains($barthelCat, 'Independen')
                                                ? 'success'
                                                : (str_contains($barthelCat, 'sedang')
                                                    ? 'warning'
                                                    : 'danger');
                                        @endphp
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($pasien->sit_to_stand)
                                        <span
                                            class="badge badge-{{ \App\Helpers\PemeriksaanHelper::isSitStandNormal($pasien->sit_to_stand, $age) ? 'success' : 'danger' }}">
                                            {{ \App\Helpers\PemeriksaanHelper::isSitStandNormal($pasien->sit_to_stand, $age) ? 'Normal' : 'Tidak Normal' }}
                                        </span>
                                    @else
                                        <span class="badge badge-secondary">Belum diisi</span>
                                    @endif
                            ...
                            <tr>
                                <td>2-Minute Step Test ({{ $pasien->step_test ?? 'Belum diisi' }} langkah)</td>
                                <td>
                                    @if ($pasien->step_test !== null)
                                        <span
                                            class="badge badge-{{ \App\Helpers\PemeriksaanHelper::isStepNormal($pasien->step_test, $age, $gender) ? 'success' : 'danger' }}">
                                            {{ \App\Helpers\PemeriksaanHelper::isStepNormal($pasien->step_test, $age, $gender) ? 'Normal' : 'Tidak Normal' }}
                                        </span>
                                    @else
                                        <span class="badge badge-secondary">Belum diisi</span>
                                    @endif
                    ...
                    <div class="col-md-6">
                        ...
                        @if ($videoKhusus->count() == 0 && $videoGlobal->count() == 0)
                            <div class="alert alert-info">
                                ...
                                <br>
                                <a href="{{ route('admin.videos.create') }}" class="btn btn-sm btn-primary mt-2">
